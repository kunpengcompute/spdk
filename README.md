...
- 压缩是使用压缩算法对用户的数据进行压缩，能够节省存储空间，如果需要经过网络传输的话也能减少网络传输的数据量。压缩使用的是DPDK库中的压缩驱动，如果需要进行KAE卸载，需要设置为zlib驱动，具体使用方式参考[用户指南](docs/zh/user_guide.md)。
...
环境要求、编译安装指南详见[安装指南](docs/zh/installation_guide.md)。

## 用户指南<a id="ZH-CN_TOPIC_0000002520989184"></a>

SPDK相关特性使用方法见[用户指南](docs/zh/user_guide.md)。
